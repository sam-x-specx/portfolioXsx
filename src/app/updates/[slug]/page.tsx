...
export async function generateMetadata({ 
  ...
  return {
    title: update.heading ?? update.title,
    description: update.description || 'Latest update and thoughts from Sambit.',
    openGraph: {
      title: update.heading ?? update.title,
      description: update.description || 'Latest update and thoughts from Sambit.',
      url: `/updates/${slug}`,
      // Add image if you have one in the future:
  ...
  return (
    <main className="min-h-screen bg-[#030712] text-white font-sans">
      <div className="max-w-2xl mx-auto px-6 py-24 flex flex-col gap-10">
        ...
        {/* ── 4. Views + tags ── */}
        <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500 -mt-4">
          <span>👁 {fmtViews(update.views ?? 0)} views</span>
          {update.tags && update.tags.length > 0 && (
            <>
              <span>·</span>
              {update.tags.map((tag: string) => (
